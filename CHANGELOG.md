...
### Changed
- CI TigerBeetle containers no longer use `--privileged`. Before starting
  containers the workflow enables `io_uring` via host-level sysctl; the
  `format` command runs with no special Docker permissions; the `start`
  command disables only the seccomp profile (`--security-opt seccomp=unconfined`)
  and grants `--cap-add=IPC_LOCK,SYS_RAWIO`. This reduces the CI attack
  surface. (#130)
