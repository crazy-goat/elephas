...
### Changed
- CI TigerBeetle containers no longer use `--privileged`; `io_uring` is enabled via host-level sysctl, the `format` command uses no special Docker permissions, and the `start` command only disables seccomp and adds `--cap-add=IPC_LOCK,SYS_RAWIO` (#130)
